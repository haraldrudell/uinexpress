...
function __express(filename, options, callback) {
	...
	getTemplate(filename, function (template) {
		...
		// if undefined or false: skip layout
		if (options.layout === undefined || (options.layout !== undefined && !options.layout)) callback(null, body)
		else {
			// get layout filename
			var layoutFile = options.layout == true ? 'layout' : (options.layout || 'layout')
